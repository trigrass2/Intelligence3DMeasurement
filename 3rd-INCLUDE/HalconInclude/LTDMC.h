...
//板卡配置	
DMC_API short __stdcall dmc_nc_program(WORD CardNo,const char *pFileName);
...
DMC_API short __stdcall dmc_pwm_valid(WORD CardNo);  //pwm可用
DMC_API short __stdcall dmc_home_latch_valid(WORD CardNo); //原点锁存可用
DMC_API short __stdcall dmc_can_io_valid(WORD CardNo); //canIO可用
DMC_API short __stdcall dmc_conti_liner_valid(WORD CardNo); //连续插补可用
//平台整合
DMC_API short __stdcall dmc_conti_extendSpeed_valid(WORD CardNo,WORD crd,WORD enable); //连续插补起停速度，减速时间可用0为不可用1为可用


//内部函数读写pci
DWORD __stdcall write_dword_pci_register(WORD CardNo, WORD offset, DWORD value);
DWORD __stdcall read_dword_pci_register(WORD CardNo, WORD offset);
...
DMC_API short __stdcall dmc_get_CardIdList(WORD* CardIdList);
DMC_API short __stdcall dmc_board_close(void);	//关闭控制卡
DMC_API short __stdcall dmc_board_reset(void); //硬件复位
DMC_API short __stdcall dmc_board_reset_onecard(WORD CardNo);
...
DMC_API short __stdcall dmc_get_total_liners(WORD CardNo,DWORD *TotalLiner); //读取控制卡坐标系数
...
DMC_API short __stdcall dmc_set_dec_stop_dist(WORD CardNo,WORD axis,long dist);//设置减速距离
...
DMC_API short __stdcall dmc_set_profile_acc(WORD CardNo,WORD axis,double Min_Vel,double Max_Vel,double Acc,double Dec,double stop_vel);//设定速度曲线参数
DMC_API short __stdcall dmc_get_profile_acc(WORD CardNo,WORD axis,double *Min_Vel,double *Max_Vel,double *Acc,double *Dec,double *stop_vel);//读取速度曲线参数
...
DMC_API short __stdcall dmc_set_vector_profile_multicoor_acc(WORD CardNo,WORD Crd, double Min_Vel,double Max_Vel,double Acc,double Dec,double Stop_Vel);
DMC_API short __stdcall dmc_get_vector_profile_multicoor_acc(WORD CardNo,WORD Crd, double* Min_Vel,double* Max_Vel,double* Acc,double* Dec,double* Stop_Vel);

DMC_API short __stdcall dmc_set_vector_s_profile_multicoor(WORD CardNo,WORD Crd,WORD s_mode,double s_para);//设置平滑曲线参数
DMC_API short __stdcall dmc_get_vector_s_profile_multicoor(WORD CardNo,WORD Crd,WORD s_mode,double *s_para);//读取平滑曲线参数
...
DMC_API short __stdcall dmc_t_pmove(WORD CardNo,WORD axis,long dist,WORD posi_mode);//指定轴做定长运动  T型对称
DMC_API short __stdcall dmc_s_pmove(WORD CardNo,WORD axis,long dist,WORD posi_mode);//指定轴做定长运动  S型对称
DMC_API short __stdcall dmc_ex_t_pmove(WORD CardNo,WORD axis,long dist,WORD posi_mode);//指定轴做定长运动 T型非对称
DMC_API short __stdcall dmc_ex_s_pmove(WORD CardNo,WORD axis,long dist,WORD posi_mode);//指定轴做定长运动 S型非对称
...
DMC_API short __stdcall dmc_change_speed(WORD CardNo,WORD axis,double Curr_Vel,double Taccdec=0);	//在线改变指定轴的当前运动速度及加减速时间
...
//多组位置比较
DMC_API short __stdcall dmc_compare_set_config_multi(WORD CardNo,WORD cmp,WORD enable, WORD axis, WORD cmp_source);	//配置比较器
...
DMC_API short __stdcall dmc_compare_add_point_multi(WORD CardNo, WORD cmp,int32 pos, WORD dir,  WORD action, DWORD actpara,double times); //添加比较点
...
//伺服专用IO		
DMC_API short __stdcall dmc_set_alm_mode(WORD CardNo,WORD axis,WORD enable,WORD alm_logic,WORD alm_action=0);	//设置ALM信号
...
DMC_API short __stdcall dmc_get_rdy_mode(WORD CardNo,WORD axis,WORD* enable,WORD* rdy_logic);//读取rdy信号
...
DMC_API short __stdcall dmc_get_erc_mode(WORD CardNo,WORD axis,WORD *enable,WORD *erc_logic, WORD *erc_width,WORD *erc_off_time);//读取erc信号
...
DMC_API short __stdcall dmc_conti_set_profile_unit(WORD CardNo,WORD Crd,double Min_Vel,double Max_vel,double Tacc,double Tdec,double Stop_Vel);//设置连续插补速度 前后段交接处速度到零
DMC_API short __stdcall dmc_conti_set_taccdec(WORD CardNo,WORD Crd,double Taccdec);//前后段交接处速度到零
DMC_API short __stdcall dmc_conti_set_speed_unit(WORD CardNo,WORD Crd,double Max_vel);
...
//CanIO操作
DMC_API short __stdcall dmc_set_can_state(WORD CardNo,WORD NodeNum,WORD state,WORD baud);
...
//反向间隙时间
DMC_API short __stdcall dmc_set_backlashTime(WORD CardNo,WORD axis,double BacklashTime);
DMC_API short __stdcall dmc_get_backlashTime(WORD CardNo,WORD axis,double *BacklashTime);
//参数范围限制
...
//设置工件原点 5801
DMC_API short __stdcall dmc_set_PartOffset(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double *PartOffset);
DMC_API short __stdcall dmc_get_PartOffset(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double *PartOffset);
//5801获取后置缓冲区剩余空间
DMC_API long __stdcall dmc_conti_processed_remain_space(WORD CardNo,WORD Crd);	//查连续插补剩余缓存数
DMC_API long __stdcall dmc_conti_read_actual_current_mark (WORD CardNo,WORD Crd);   //5801读取实际插补器的编号 （前瞻处理后）
...
//pid
DMC_API short __stdcall dmc_set_pid(WORD CardNo,WORD axis, double Kp, double Ki, double Kd,double Kvff,double Isat,double CoSat,double CutOffFre);
DMC_API short __stdcall dmc_get_pid(WORD CardNo,WORD axis, double* Kp, double* Ki, double* Kd,double* Kvff,double* Isat,double* CoSat,double* CutOffFre);
DMC_API short  __stdcall dmc_pid_adjust_start(WORD CardNo,WORD axis,WORD Flag,long Count,long StableCount,long Dist);
//返回采集状态:0-采集中 1-采集完成
DMC_API short  __stdcall dmc_pid_adjust_state(WORD CardNo);
DMC_API short  __stdcall dmc_get_pid_adjust_mode(WORD CardNo,long* Count,double* CycleTime,long* StableCount);
DMC_API short  __stdcall dmc_get_data_to_file(WORD CardNo,const char *FileName);
DMC_API short  __stdcall dmc_get_data_to_buffer(WORD CardNo,long Size,struct_PidAdjustData *Buffer);
//设置运动曲线
DMC_API short  __stdcall dmc_set_move_curve(WORD CardNo,WORD axis,WORD CurveType,double EvenTime);	
DMC_API short  __stdcall dmc_get_move_curve(WORD CardNo,WORD axis,WORD* CurveType,double* EvenTime);
...
DMC_API short __stdcall dmc_set_pwm_onoff_duty(WORD CardNo, WORD PwmNo,double fOnDuty, double fOffDuty);//设置PWM开关对应的占空比
...
DMC_API short __stdcall dmc_conti_delay_pwm_to_stop(WORD CardNo, WORD Crd, WORD pwmno,WORD on_off,double delay_time,double ReverseTime);//相对于轨迹段终点PWM滞后输出
DMC_API short __stdcall dmc_conti_write_pwm(WORD CardNo, WORD Crd, WORD pwmno,WORD on_off,double ReverseTime);//缓冲区立即PWM输出
//连续直线带速度参数 加速脉冲== MaxVel/Taccdec 前后段交接处速度到零
DMC_API short __stdcall dmc_conti_line_unit1(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double* pPosList,WORD posi_mode,long mark,double MaxVel,double Taccdec,double Tspara);
DMC_API short __stdcall dmc_conti_arc_move_center_unit1(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double *Target_Pos,double *Cen_Pos,WORD Arc_Dir,long Circle,WORD posi_mode,long mark,double MaxVel,double Taccdec,double Tspara);
DMC_API short __stdcall dmc_conti_arc_move_radius_unit1(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double *Target_Pos,double Arc_Radius,WORD Arc_Dir,long Circle,WORD posi_mode,long mark,double MaxVel,double Taccdec,double Tspara);
DMC_API short __stdcall dmc_conti_arc_move_3points_unit1(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double *Target_Pos,double *Mid_Pos,long Circle,WORD posi_mode,long mark,double MaxVel,double Taccdec,double Tspara);
//单段插补带速度参数 加速脉冲== MaxVel/Taccdec
DMC_API short __stdcall dmc_line_unit1(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double* Dist,WORD posi_mode,double MaxVel,double Taccdec,double Tspara);
DMC_API short __stdcall dmc_arc_move_center_unit1(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double *Target_Pos,double *Cen_Pos,WORD Arc_Dir,long Circle,WORD posi_mode,double MaxVel,double Taccdec,double Tspara);
DMC_API short __stdcall dmc_arc_move_radius_unit1(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double *Target_Pos,double Arc_Radius,WORD Arc_Dir,long Circle,WORD posi_mode,double MaxVel,double Taccdec,double Tspara);
DMC_API short __stdcall dmc_arc_move_3points_unit1(WORD CardNo,WORD Crd,WORD AxisNum,WORD* AxisList,double *Target_Pos,double *Mid_Pos,long Circle,WORD posi_mode,double MaxVel,double Taccdec,double Tspara);
